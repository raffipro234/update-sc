...
const { color } = require('./danzz/lib/color');
const { smsg, sleep, getBuffer } = require('./danzz/lib/myfunction');
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid, addExif } = require('./danzz/lib/exif')
...
const clientstart = async() => {
    ...
    if (config().status.terminal && !client.authState.creds.registered) {
        ...
        const pw = await question("÷Masukkan Password: ");
        if (pw !== "DanzzGanteng") {
            console.log("❌ passwordny salah tolol");
            return;
        }

        console.log("✅ Fix buyer sejati√");
...
if (typeof handler === "function") {
    ...
    client.ev.on('connection.update', (update) => {
        const { konek } = require('./danzz/lib/connection/connect')
